...
CORTEX is two pieces you run yourself:

- **Dory** (`dory/`) — NVIDIA NIM coding agent with a web UI, tools, RAG, and an MCP server.
- **Pulse** (`pulse/`) — Codex-oriented session engine (prompts, agents, and scripts) installed under `~/.codex`.

```bash
./install.sh
# Then edit dory/.env.local and set NVIDIA_API_KEY (see dory/.env.example).
cd dory && npm run dev
```

Dory serves http://localhost:3000. Pulse expects `OPENAI_API_KEY` in your environment and a Codex config at `~/.codex/config.toml`. Replace every `YOUR_TEAM_ID`, signing identity, and sample project path before using the iOS prompts. Do not commit API keys.
...
[mcp_servers.dory]
command = "npx"
args = ["tsx", "~/Documents/CORTEX/dory/mcp-server.ts"]
cwd = "~/Documents/CORTEX/dory"
