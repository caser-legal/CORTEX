...
<p align="center">
  ...
  <em>Powered by NVIDIA NIM • 44 Custom Tools • 1M Token Context • RAG + Memory</em>
</p>
...
- **44 hand-built tools** for file operations, code search, web research, memory, and more
...
- **Agent system** - The AI can use 44 tools to accomplish tasks
...
A standalone server (`mcp-server.ts`) that exposes all 44 tools via the **Model Context Protocol**. This means:
...
// AI says: "I need to read AppDelegate.swift"
{
  ...
  "arguments": {
    "operation": "read",
    "path": "~/Documents/iOS/MyApp/AppDelegate.swift"
  }
}
...
2. **Exposes all 44 tools** - Via JSON-RPC over stdio
...
command = "npx"
args = ["tsx", "~/Documents/nvidia-cli/mcp-server.ts"]
cwd = "~/Documents/nvidia-cli"
startup_timeout_sec = 120
tool_timeout_sec = 120
env = { NGC_API_KEY = "${NGC_API_KEY}", NVIDIA_API_KEY = "${NGC_API_KEY}" }
```

Now when you run `codex`, it has access to all 44 tools!
...
- Chat with an AI that has access to 44 tools
...
- Use the same 44 tools in your terminal
...
1. **MCP Server** (`mcp-server.ts`) exposes all 44 tools via JSON-RPC over stdio
...
| File | Purpose |
|------|---------|
| `mcp-server.ts` | MCP server exposing 44 tools (runs with `npx tsx`) |
...
cd ~/Documents/nvidia-cli
npx tsx mcp-server.ts

# In Codex CLI, check tools are available
/mcp
# Should show nvidia-cli with 44 tools
```
...
**Kiro CLI** is AWS's AI-powered coding assistant that supports MCP servers and custom agents. The **Dory agent** configuration connects Kiro to nvidia-cli's 44 tools plus Context7 for live documentation lookup.
...
## Project Context
- **Current project**: iOS SpaceX App (ExampleApp)
- **Project path**: ~/Documents/iOS/ExampleApp
...
alias nv="~/Documents/nvidia-cli/start.sh"
alias codex="~/Documents/nvidia-cli/start.sh"
...
1. **nvidia-cli MCP Server** - 44 tools for file ops, RAG, search, memory
...
flowchart TB
    ...
    subgraph Tools["🛠️ Available Tools"]
        ...
        FLY[flywheel_*]
        DORY[dory_agent]
        VISION[vision_analyze, ios_ui_review]
        SPEC[search_specialist, report_writer]
        RESOLVE[resolve-library-id]
        GETDOCS[get-library-docs]
    end
...
### dory_agent - Full Orchestration Pipeline
...
**When to use:**
- Complex tasks that need the full agent workflow
- Tasks requiring multiple tool calls
- When you want automatic context gathering

**For simple tasks**, use individual tools directly (`file_read`, `bash`, etc.)

### Vision Analysis Tools (3)

| Tool | Description |
|------|-------------|
| `vision_analyze` | Analyze images with NVIDIA VLM (Nemotron Nano VL 12B v2) |
| `ios_ui_review` | Review iOS screenshots for UI issues (alignment, spacing, accessibility) |
| `compare_mockup` | Compare Figma mockup to implementation |

### Specialist Agent Tools (9)

| Tool | Description |
|------|-------------|
| `search_specialist` | Deep multi-source research |
| `report_writer` | Create full report from research |
| `quality_reviewer` | Evaluate report quality (0-10 scores) |
| `report_extender` | Extend report with new findings |
| `deduplicate_sources` | Clean up source citations |
| `report_planner` | Structured outline creation |
| `section_author` | Individual section writing |
| `report_compiler` | Final assembly |
...
| Tool | Description |
|------|-------------|
| `dory_agent` | Full orchestration pipeline (RAG + Memory + Tools + Flywheel) |
...
| `flywheel_stats` | Get flywheel statistics |
| `flywheel_export` | Export flywheel data for training |
| `flywheel_create_dataset` | Create training datasets |
...
1. **nvidia-cli MCP Server** - 44 tools for file ops, RAG, search, memory, vision
2. **Context7 MCP Server** - Live documentation lookup
3. **Loads user-memory.md** - Via agentSpawn hook
4. **Claude Opus 4.5** - As the reasoning model
