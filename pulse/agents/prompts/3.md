Developer: ## 🚨 NEVER ASSUME - ALWAYS CHECK MCP TOOLS (GOOGLE RESEARCH & SEARCH / CONTEXT7 & PERPLEXITY FIRST & AFTER ALL ISSUES MARK ALL FALSE POSITIVES IN PREFLIGHT AS COMPLETE DO NOT KEEP RUNNING NEW SESSIONS! ALWAYS FOCUS ON  Missing QA checks! IGNORE ANY VoiceOver or ACCESSIBILITY ISSUES MARK ALL AS COMPLETE AND MOVE ON TO REAL FEATURES! 🚨

Always verify assumptions:
1. **Try the operation first**—run the command, inspect files, or test the tool.
2. **Do not assume failure without direct evidence.**
3. **If considering 'manual steps,' attempt them before suggesting.**

Always check by doing—never assume.
...
### 1.4 Verify NO Star Button on Main Screen
```bash
# Check for star/crown buttons outside Settings (SHOULD NOT EXIST)
grep -rn "star.fill\|crown.fill" --include="*.swift" . | grep -v Settings | grep -v Paywall
```
**If found in ContentView/MainView/HomeView, REMOVE IT. Paywall is Settings-only!**
...
- Build for physical device (`-destination 'generic/platform=iOS'`).
- Treat warnings as errors—fix all.
- Build must succeed with zero errors and warnings.
- Install on device if possible.
...
- [ ] **NO star/crown button on main screen** (paywall is Settings-only!)
...
- [ ] Icon buttons: accessibility labels
